...
CREATE TABLE IF NOT EXISTS locale_suppl_product_keyword (
  locale_suppl_prod_keyword_id INT NOT NULL AUTO_INCREMENT  COMMENT 'Unique Identifier for the supplier category table',
...
--   merg_suppl_prod_keywrd_id INT NOT NULL  COMMENT 'Unique Identifier for merge supplier product keyword',
 --  status  CHAR(1) NOT NULL DEFAULT 'A' COMMENT 'record status',
  ...
  CONSTRAINT PK_locale_suppl_product_keyword PRIMARY KEY (locale_suppl_prod_keyword_id),
  CONSTRAINT UQ_locale_suppl_product_keyword UNIQUE KEY (suppl_product_id, suppl_prod_keyword_id, locale_id),
  CONSTRAINT fk_locale_suppl_prod_keyword_prod
  FOREIGN KEY ( suppl_product_id )
  REFERENCES supplier_catalog.suppl_product (  suppl_product_id  )
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
  CONSTRAINT fk_locale_suppl_prod_keyword
  FOREIGN KEY ( suppl_prod_keyword_id )
  REFERENCES supplier_catalog.suppl_product_keyword ( suppl_prod_keyword_id  )
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,  
  CONSTRAINT fk_locale_suppl_prod_keyword_suppl_prod
  FOREIGN KEY ( locale_suppl_product_id)
  REFERENCES supplier_catalog.locale_suppl_product ( locale_suppl_product_id)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,  
  CONSTRAINT fk_locale_sppl_prod_keyword_locale
