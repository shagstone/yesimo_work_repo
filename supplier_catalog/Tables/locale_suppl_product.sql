...
CREATE TABLE IF NOT EXISTS locale_suppl_product (
  locale_suppl_product_id INT NOT NULL AUTO_INCREMENT  COMMENT 'Unique Identifier for the supplier category table',
...
--   merg_suppl_prod_id INT NOT NULL COMMENT 'merge supplier product table unique identifier',
  ...
  CONSTRAINT PK_suppl_product_locale PRIMARY KEY (locale_suppl_product_id),
  CONSTRAINT UQ_suppl_product_locale UNIQUE KEY (suppl_product_id, locale_id),
...
--   REFERENCES supplier_catalog.mlocale_suppl_prod (merg_suppl_prod_id, locale_id )
--   ON DELETE NO ACTION
--   ON UPDATE NO ACTION
  )
