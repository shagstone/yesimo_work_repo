...
DROP TABLE IF EXISTS suppl_product_inventory ;


CREATE TABLE IF NOT EXISTS suppl_product_inventory (
 ...
 --  status  CHAR(1) NOT NULL DEFAULT 'A' COMMENT 'record status',
  ...
  CONSTRAINT PK_suppl_prod_inv PRIMARY KEY ( suppl_product_id),
  CONSTRAINT UQ_suppl_prod_inv UNIQUE KEY ( supplier_id, suppl_prod_num),
  CONSTRAINT UQ_suppl_prod_inv_checksum UNIQUE KEY (suppl_prod_inv_checksum),
  CONSTRAINT fk_suppl_prod_inv_supplier
  FOREIGN KEY (supplier_id )
  REFERENCES supplier_catalog.supplier (supplier_id )
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
  CONSTRAINT fk_suppl_prod_inv_suppl_prod
