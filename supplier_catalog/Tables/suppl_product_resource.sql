...
CREATE TABLE IF NOT EXISTS suppl_product_resource (
 ...
 --  status  CHAR(1) NOT NULL DEFAULT 'A' COMMENT 'record status',
  ...
  CONSTRAINT PK_suppl_product_rsrc PRIMARY KEY (suppl_product_rsrc_id),
  FOREIGN KEY (suppl_product_id )
  REFERENCES supplier_catalog.suppl_product (suppl_product_id )
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
  FOREIGN KEY (resource_type_id )
  REFERENCES supplier_catalog.rsrc_type (rsrc_type_id )
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,
  CONSTRAINT fk_suppl_prod_rsrc_suppl_prod_num
...
--   REFERENCES supplier_catalog.rsrc_subtype (rsrc_subtype_id )
--   ON DELETE NO ACTION
--   ON UPDATE NO ACTION
)
