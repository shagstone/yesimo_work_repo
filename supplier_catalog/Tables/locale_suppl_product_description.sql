...
CREATE TABLE IF NOT EXISTS locale_suppl_product_description (
 ...
 --  status  CHAR(1) NOT NULL DEFAULT 'A' COMMENT 'record status',
  ...
  CONSTRAINT PK_locale_suppl_product_descrip PRIMARY KEY (suppl_product_id, suppl_prod_descrip_id, locale_id),
  CONSTRAINT fk_locale_suppl_product_description
  FOREIGN KEY (suppl_product_id, suppl_prod_descrip_id )
  REFERENCES supplier_catalog.suppl_product_description  (suppl_product_id, suppl_prod_descrip_id)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION,  
  CONSTRAINT fk_sppl_prod_descrip_suppl_prod
