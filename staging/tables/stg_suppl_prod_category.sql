...
-- -----------------------------------------------------
-- Table staging.stg_suppl_product_category
-- -----------------------------------------------------
...
DROP TABLE IF EXISTS staging.stg_suppl_product_category ;

 
CREATE TABLE staging.stg_suppl_product_category(
  stg_suppl_supplier_category_id int  AUTO_INCREMENT  NOT NULL COMMENT 'Unique identifier for the stg_suppl_product_category',
  ...
  CONSTRAINT PK_stg_suppl_prod_category PRIMARY KEY ( stg_suppl_supplier_category_id),
  CONSTRAINT fk_stg_suppl_prod_category_supplier
  FOREIGN KEY (supplier_id)
  REFERENCES supplier_catalog.supplier (supplier_id)
  ON DELETE NO ACTION
  ON UPDATE NO ACTION
 ) 
ENGINE=InnoDB DEFAULT CHARSET=utf8, COMMENT = 'Staging Supplier Product Category Table';
